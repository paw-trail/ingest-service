...
public enum IngestErrorCode implements ErrorCode {
    ...
    // 같은 소스의 받아 오기를 마친 지 얼마 안 됨
    //
    // * 왜 막는가
    //   관리자 화면의 「최신 수집 실행」 은 여러 사람이 번갈아 누를 수 있음
    //   실행 중 잠금은 도는 동안만 막아, 끝나자마자 다시 누르면 또 돎
    //   반려동물 동반여행은 한 번에 상세 기능마다 표본 20회를 쓰므로
    //   짧은 간격으로 계속 누르면 그날 허용량이 바닥남
    //
    // * 429 인 이유
    //   지금 상태와 충돌하는 것이 아니라 요청이 너무 잦은 것임
    //   user 의 SUMMARY_COOLDOWN · auth 의 MAIL_SEND_COOLDOWN 과 같은 자리
    //   409 를 다시 쓰면 도는 것이 없는데 「이미 실행 중」 이라고 안내하게 됨
    //
    // * 관리자 입구와 매일 예약에만 걸림
    //   /internal 트리거는 사람이 전량을 다시 받거나 넘기기를 부르는 자리라 걸지 않음
    //   다만 셀 때는 /internal 로 건 실행도 함께 셈 — 허용량은 누가 불렀든 같이 줄어듦
    //
    // * 얼마나 막는지는 설정 app.ingest.cooldown 이 정함 (기본 10분)
    //   그래서 문구에 시간을 적지 않음
    INGEST_COOLDOWN(HttpStatus.TOO_MANY_REQUESTS, "최근에 같은 소스를 수집했습니다. 잠시 후 다시 실행해 주세요."),
    ...
    // 그 소스로는 할 수 없는 실행 유형임
    //
    // * 넷을 거름 — 트리거 서비스의 조합 검사
    //   증분을 지원하지 않는 소스의 INCREMENTAL (고캠핑 · 문화정보원)
    //   원본을 담지 않는 소스의 LINK 와 받아 오기 (행정안전부는 DIRECT 만 됨)
    //   원본을 담는 소스의 DIRECT
    //   막지 않으면 증분으로 불러도 조용히 전량이 돌아 호출 허용량을 통째로 쓰고,
    //   그것이 버그라는 것도 드러나지 않음
    //
    // * 501 인 이유
    //   위와 같음. 그 소스에 그 동작을 만들지 않은 것이지 요청 값이 틀린 것이 아님
    //   관리자 입구가 받지 않는 조합은 이 코드가 아니라 INGEST_RUN_NOT_ALLOWED(400)로 거절함
    RUN_TYPE_NOT_SUPPORTED(HttpStatus.NOT_IMPLEMENTED, "아직 지원하지 않는 실행 유형입니다."),

    // 관리자 입구로는 걸 수 없는 조합임
    //
    // * 받는 것은 둘뿐임 — 반려동물 동반여행 증분 · 고캠핑 목록
    //   둘 다 한국관광공사 OpenAPI 를 가장 적게 부르는 받아 오기이고 원문 적재에서 끝남
    //   반려동물 동반여행 전량은 상세를 3천 회 넘게 불러 그날 허용량을 통째로 씀
    //   LINK · DIRECT 는 장소 서비스로 넘기는 실행이라 이 입구가 약속한 「적재까지만」 을 넘음
    //   문화정보원 · 행정안전부는 공사 API 가 아니라 레포에 담은 파일을 읽음
    //
    // * 400 인 이유
    //   값은 우리가 정의한 것이고 /internal 로는 되는 조합임 — 이 경로에서만 안 되는 것임
    //   경로마다 받는 값이 다를 때 400 으로 알리는 policy 의 POLICY_SOURCE_NOT_ALLOWED 를 따름
    //   RUN_TYPE_NOT_SUPPORTED(501)는 「아직 구현하지 않음」 이라 뜻이 맞지 않음
    //   반려동물 동반여행 전량은 구현돼 있음
    //
    // * 10분 잠금 · 실행 중 검사보다 먼저 봄
    //   안 되는 조합에 「잠시 후 다시」 가 나가면 기다려도 안 된다는 것을 알 수 없음
    INGEST_RUN_NOT_ALLOWED(
            HttpStatus.BAD_REQUEST, "관리자 화면에서는 반려동물 동반여행 증분과 고캠핑 목록만 실행할 수 있습니다."),
