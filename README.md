...
`extract` 는 아직 만들지 않았습니다. 이 서비스가 담아 둔 17,471건이 그 서비스의 재료입니다.
...
| 위 소스 셋 | `LINK` | 위가 끝난 뒤. **받은 것이 바뀌었을 때만 의미가 있음** |
| `MOIS_VET` | `DIRECT` | 파일을 바꿨을 때만. **`LINK` 를 따로 안 부릅니다** |

> ⛔**소스마다 되는 실행 종류가 정해져 있습니다.** 어긋나면 501 로 거절합니다.
> 조합 표는 [5-1](#5-1-post-internalingesttrigger--수집을-시작) 에 있고
> Jenkins 잡을 만들 때 그 표를 봐야 합니다.
