...
행정안전부 CSV  ──────────────▶  ⛔raw_db 를 안 거치고 place 로 바로 감
```
...
**소스 하나만 담지 않고 바로 보냅니다.** 행정안전부 동물병원 인허가입니다.
동반 조건 문장이 없어 해석할 것도 원문으로 보여줄 것도 없기 때문이며,
그래서 실행 종류가 하나 더 있습니다. [2-6](#2-6-mois_vet--담지-않고-바로-보냅니다) 을 보십시오.
...
그 정보는 공공데이터포털에 흩어져 있습니다.
한국관광공사가 두 벌, 한국문화정보원이 한 벌, 행정안전부가 한 벌을 냅니다.
...
`extract` 가 이 서비스가 담아 둔 원문을 가져가 조건으로 바꿉니다. 이 서비스는 해석하지 않습니다.

---

**④ 브라우저가 부르는 입구는 관리자 입구 하나뿐입니다**

```
다른 서비스     브라우저 ──▶ 게이트웨이 ──▶ 서비스        공개 경로가 여럿
이 서비스       브라우저 ──▶ 게이트웨이 ──▶ 관리자 입구   ADMIN 만 · 수집 실행과 기록 둘
                그 밖은 전부 /internal — extract · place 와 사람이 부름
```

관리자 화면 「운영」의 「공사 데이터 최신 수집」 카드가 부르는 입구만 게이트웨이를 거칩니다.
ADMIN 계정만 통과하며, 수집을 걸고 기록을 보는 것이 전부입니다 —
[5-6](#5-6-post-apiv1adminingestruns--관리자가-수집을-걸기) · [5-7](#5-7-get-apiv1adminingestruns--관리자가-수집-기록-보기).
나머지는 전부 `/internal` 이라 브라우저에서 닿지 않고 같은 VPC 안에서만 부를 수 있습니다.

로컬 compose 에서는 `app` 이 아니라 **`pipeline` 프로파일**에 있습니다.
프로파일은 「어느 컨테이너를 띄울지 고르는 태그」이고, 평소 조합에 `pipeline` 을 안 넣으면 뜨지 않습니다.

**배포에서는 늘 떠 있어야 합니다.** 관리자 버튼 · 매일 예약 수집([9-2](#9-2-수집을-언제-부르나)) ·
장소 상세의 원문보기가 이 서비스를 부르기 때문입니다.
...
스물다섯 개 컬럼이 전부 사업장명·주소·영업상태·좌표·전화·면적이라,
표시용 본문을 조립해도 원문이 아니라 우리가 만든 표가 됩니다.

그래서 그 소스만 `raw_db` 를 거치지 않고 `place` 로 바로 갑니다.
받아 오기와 넘기기가 한 번에 일어나며 실행 종류도 따로입니다.

> ⚠**그래서 동물병원은 원문보기가 영영 빈 목록입니다.** 오류가 아니라 정상입니다.
> 화면이 "이 장소는 원문 문서가 없습니다" 를 보여주어야 합니다.
...
| 지금 하려는 일 | 볼 곳 |
|---|---|
| **처음 본다** | [2장](#2-소스-4종이-서로-다릅니다) → [3장](#3-호출-허용량이-이-서비스를-지배합니다) |
| 로컬에서 띄워 본다 | [1장](#1-로컬에서-띄우기) |
| API 를 부른다 | [5장](#5-api-7개) |
...
### 1-3. CSV 파일 둘이 있어야 합니다

소스 넷 중 둘은 바깥을 부르지 않고 **레포에 함께 커밋된 파일**을 읽습니다.

```
ingest-service/data/culture/culture-facility-20250324.csv     30.6MB   UTF-8
ingest-service/data/mois/mois-vet-20260908.csv                 2.7MB   ⛔CP949
```

`git clone` 하면 함께 받아집니다. 없으면 그 소스 수집이 시작하자마자 죽습니다.
...
```
출처       공공데이터포털 15045050
           [바로가기] → 「인허가정보_동물」 → 「동물_동물병원」 → ⛔[전국 파일 다운로드]
원본명      동물_동물병원.csv
크기       2,782,217
sha256     9E7E34C9EC0BF2372531A7CDEDCF107DAD58DD93147B4B74D4853F4A271EB4D8
인코딩      ⛔CP949.  문화정보원과 다릅니다
```

> ⛔**직접 주소로는 받을 수 없고 포털 버튼을 거쳐야 합니다.** 403 이 돌아옵니다.
> **`[지역별 파일 다운로드]` 도 쓰지 마십시오.** 열일곱 개를 합쳐야 하고 서식이 갈릴 수 있습니다.

> ⚠**행정안전부 판은 매일 갱신됩니다.** 커밋된 것은 받은 날의 판입니다.
> 「지금 영업 중인 목록」이 이 소스를 쓰는 이유의 절반인데 그 절반이 커밋 시점에 묶입니다.
> 받는 절차를 만들 때 같은 데이터의 OpenAPI 판(15154952)을 검토할 자리입니다.
...
**받아 오기** — `runType` 이 `FULL` 이거나 `INCREMENTAL` 일 때입니다.
바깥을 부르므로 허용량을 씁니다.
...
| `MOIS_VET` | — | ⛔이 실행을 못 부릅니다. 아래 `DIRECT` 를 쓰십시오 |

**넘기기** — `runType` 이 `LINK` 일 때입니다.
바깥을 부르지 않고 담아 둔 것을 `place` 로 보냅니다.

| | |
|---|---|
| 쓰는 호출 | **0회.** ✅허용량과 무관해 몇 번이든 돌려도 됩니다 |
| ⛔먼저 할 것 | **`place` 를 띄워 두십시오.** 안 떠 있으면 `502` `PLACE_LINK_FAILED` 로 끝납니다 |
| 걸리는 시간 | 1,000건에 20초쯤. `CULTURE_CSV` 13,399건이면 5분 남짓 |
| 되는 소스 | 원본을 담는 셋뿐입니다. `MOIS_VET` 에 부르면 `501` 입니다 |
| 여러 번 돌리면 | 같은 결과가 됩니다. 이미 이어 둔 장소를 다시 찾아 붙어 식별자가 바뀌지 않습니다 |

**바로 보내기** — `runType` 이 `DIRECT` 일 때입니다.
파일을 읽어 그 자리에서 `place` 로 보냅니다. 우리 표를 거치지 않습니다.

| | |
|---|---|
| 쓰는 호출 | **0회.** 파일만 읽습니다 |
| ⛔먼저 할 것 | **`place` 를 띄워 두십시오.** 위와 같습니다 |
| 걸리는 시간 | 5,474건에 2분쯤 |
| 되는 소스 | **`MOIS_VET` 뿐입니다.** 나머지 셋에 부르면 `501` 입니다 |
| 여러 번 돌리면 | 같은 결과가 됩니다. 관리번호가 그대로라 장소가 다시 만들어지지 않습니다 |

> **`LINK` 와 `DIRECT` 는 정확히 반대입니다.**
> 원본을 담는 소스는 `LINK` 로 넘기고, 안 담는 소스는 `DIRECT` 로 바로 보냅니다.
> 그래서 되는 소스가 서로 겹치지 않습니다.
...
`status` 가 `DONE` 이고 `fetchedCount` 가 `13399` 이면 제대로 돈 것입니다.

**동물병원을 넣어 보려면 `DIRECT` 입니다.**

```powershell
Set-Content -Path trigger.json -Value '{"source":"MOIS_VET","runType":"DIRECT"}' -Encoding utf8 -NoNewline
curl.exe -s -X POST "http://localhost:8088/internal/ingest/trigger" -H "Content-Type: application/json" -d "@trigger.json"
```
...
curl -s -X POST "http://localhost:8088/internal/ingest/trigger" \
  -H "Content-Type: application/json" \
  -d '{"source":"MOIS_VET","runType":"DIRECT"}'
```

> ⛔**이 실행은 `fetchedCount` 가 0 으로 남습니다.** 실패가 아닙니다.
> 그 값은 우리 표에 담은 건수인데 이 소스는 담지 않습니다.
> 몇 건을 보냈는지는 로그에 남고, 실제로 들어갔는지는 `place` 에서 봅니다.
...
아래 셋은 **원본을 우리 표에 담는 소스**입니다.
넷째인 `MOIS_VET` 은 성격이 달라 [2-6](#2-6-mois_vet--담지-않고-바로-보냅니다) 에 따로 적었습니다.
...
### 2-6. `MOIS_VET` — 담지 않고 바로 보냅니다

**넷째 소스입니다. 앞의 셋과 다른 길로 갑니다.**

```
앞의 셋      파일이나 API 를 읽음  ──▶  raw_document 에 담음
                                        │
                                        └──▶  나중에 LINK 로 place 에 넘김

MOIS_VET     파일을 읽음  ──▶  ⛔그 자리에서 place 로 보냄
                                우리 표를 거치지 않음
```
...
|---|---|
| 무엇 | 행정안전부 동물병원 인허가 |
| 어디서 | ⛔로컬 파일 2.7MB · CP949 |
| 호출 수 | **0회** |
| 파일 행 수 | 10,618 |
| 보내는 건수 | 5,474 |
| 식별자 | `관리번호`. 열여덟 자리이고 전부 고유합니다 |
| 증분 | ⛔미지원 |
| 실행 종류 | ⛔`DIRECT` 하나뿐입니다 |
| 걸리는 시간 | 2분쯤 |
...
**담지 않는 이유는 담아 둘 값이 없기 때문입니다.**

원본을 보관하는 까닭이 둘인데 이 소스는 둘 다 해당하지 않습니다.

```
① extract 가 다시 해석할 재료      ⛔동반 조건 문장이 없어 뽑을 것이 없음
② 원문보기가 보여줄 근거           ⛔스물다섯 컬럼이 전부 정형값임
```

표시용 본문을 만들면 이런 것이 나옵니다.
...
**이것은 원문이 아니라 우리가 만든 표입니다.**
[4-2](#4-2-display_body--사람이-읽는-것만) 에서 정한 「사람이 읽는 자연어만」 기준에 걸립니다.
그리고 원문보기의 목적이 「우리 판정의 근거를 직접 보는 것」인데
이 소스는 판정에 쓰이는 값이 하나도 없습니다.
동반 조건은 문화정보원이 주고 이쪽은 폐업 여부와 좌표만 줍니다.
...
영업/정상             5,474   51.6%   ✅보냄
폐업                 5,057   47.6%
취소/말소/만료/정지/중지     59    0.6%
휴업                   28    0.3%   ⛔안 보냄
```

휴업을 빼는 이유는 받는 쪽에 **그 상태를 담을 값이 없기 때문**입니다.
장소의 상태가 운영과 폐업 둘뿐이라 어느 쪽에 넣어도 거짓말이 됩니다.
값을 늘리면 화면과 필터와 검색 색인이 따라오는데 스물여덟 건이 그럴 무게가 아닙니다.
재개업하면 `영업/정상` 이 되어 다음 실행에 들어옵니다.

---

**⛔좌표계가 다릅니다.** 이 소스만 위경도가 아닙니다.

```
좌표정보(X) "199947.178659037    "   ⛔미터 단위 평면 좌표.  값 뒤에 공백도 붙어 옴
좌표정보(Y) "453593.826987348    "
```

`EPSG:5174` 입니다. 보정계수가 들어가지 않은 Bessel 타원체 중부원점 횡메르카토르입니다.
`Epsg5174Converter` 가 위경도로 옮겨 보냅니다.

> ⛔**받는 쪽에 맡길 수 없습니다.** `place` 는 받은 좌표가 대한민국 범위 안인지부터 보는데,
> 미터 단위 값은 그 범위에 안 들어가 「쓸 수 없는 좌표」로 판정됩니다.
> 그러면 5,258건이 전부 주소 지오코딩으로 넘어가 소스가 준 측량 좌표가 버려집니다.

옮긴 뒤의 정확도는 재 보았습니다.
같은 주소를 가진 기존 장소와 거리를 잰 1,783쌍에서 **중앙값이 3m, 90분위가 20m** 였습니다.
원본 좌표끼리도 나는 차이라 `place` 가 원본과 같은 100m 기준으로 병합합니다.

---

**전화번호는 지역번호가 있는 것만 보냅니다.**

```
 8자리    115   "36728441"       ⛔지역번호가 없어 걸 수 없음
 9자리    396   "027449098"      ⛔같음
10자리  2,449   ✅보냄
11자리     87   ✅보냄.  070 인터넷전화
```

소재지 시도로 앞자리를 유추하는 것은 추정이라 하지 않습니다.
문화정보원이 전화를 96.7% 주고 형식도 정상이라 그쪽이 대부분을 채웁니다.
받는 쪽이 빈 칸만 메우므로 이 값은 그쪽에 없는 병원에만 들어갑니다.

> 하이픈을 넣지 않고 숫자 그대로 보냅니다.
> 어디에 넣을지는 지역번호 자릿수를 알아야 정해지고 그것은 표시 형식입니다.

---

**좌표가 없는 216건도 그대로 보냅니다.**

상세주소가 긴 신축 건물들입니다.
좌표를 만들 수 있는지 정하는 규칙이 `place` 안에 있어 그쪽이 판단합니다.
지오코딩으로 채우거나, 주소가 같은 짝의 좌표를 물려받거나, 둘 다 안 되면 건너뜁니다.
문화정보원과 고캠핑도 같은 방식이었습니다.
...
`extract` 가 처리 결과를 되돌려 쓸 때도 이 값을 함께 보냅니다.
가져간 사이에 재수집이 내용을 바꿨으면 해시가 달라 상태가 바뀌지 않습니다 — [5-4](#5-4-patch-internalrawstatus--처리-결과-되돌려-쓰기)
...
그러면 `extract` 가 17,471건을 통째로 다시 돕니다.

확인하는 방법은 간단합니다. **같은 수집을 두 번 돌려 두 번째가 `changed=0` 이면 됩니다.**

```
1차   fetched 13399 · changed 13399
2차   fetched 13399 · changed 0        ✅규칙이 지켜지고 있음
```
...
**`/internal` 다섯과 관리자 입구 둘입니다.**
다섯은 서비스끼리와 사람이 부르고, 둘은 게이트웨이를 거쳐 관리자 화면이 부릅니다.

| | 경로 | 누가 부르나 |
|---|---|---|
| 1 | `POST /internal/ingest/trigger` | 사람 — 전량 · 넘기기 · 바로 보내기처럼 관리자 입구가 받지 않는 실행 |
| 2 | `GET /internal/ingest/runs` | 사람 — 넘기기 · 바로 보내기까지 전부 보는 실행 이력 |
| 3 | `GET /internal/raw` | `extract` |
| 4 | `PATCH /internal/raw/status` | `extract` |
| 5 | `GET /internal/raw/{placeId}/documents` | `place` |
| 6 | `POST /api/v1/admin/ingest/runs` | 관리자 화면 — 두 조합만 · ADMIN |
| 7 | `GET /api/v1/admin/ingest/runs` | 관리자 화면 — 공사 API 를 부른 받아 오기만 · ADMIN |

⛔**`/internal` 다섯은 토큰도 헤더도 필요 없습니다.** 공통 모듈이 `/internal` 을 인증 없이 열어 두고,
실질적인 방어는 게이트웨이와 보안그룹이 맡습니다. 같은 VPC 안에서만 닿기 때문입니다.

**관리자 입구 둘은 반대로 게이트웨이를 거쳐야만 닿습니다.**
게이트웨이가 로그인 토큰을 확인해 계정과 역할을 헤더로 붙이고, 공통 모듈이 `/api/v1/admin/**` 을 ADMIN 만 통과시킵니다.
이 서비스의 컨트롤러는 역할을 따로 보지 않습니다.
...
| 값 | 넣을 수 있는 것 |
|---|---|
| `source` | `PET_TOUR` · `GOCAMPING` · `CULTURE_CSV` · `MOIS_VET` |
...
DIRECT               파일을 읽어 바로 place 로 보냄     허용량을 안 씀
                     ⛔raw_db 를 거치지 않음
```

앞의 둘은 받아 오는 방법이 다른 것이고 뒤의 둘은 하는 일이 다릅니다.
그런데도 같은 경로로 받는 것은 **실행 기록이 하나이기 때문**입니다.
언제 시작해 몇 건을 다루고 어떻게 끝났는지를 같은 표에 남깁니다.

**소스마다 되는 것이 정해져 있습니다.**

| | `FULL` | `INCREMENTAL` | `LINK` | `DIRECT` |
|---|---|---|---|---|
| `PET_TOUR` | ✅ | ✅ | ✅ | ⛔ |
| `GOCAMPING` | ✅ | ⛔ | ✅ | ⛔ |
| `CULTURE_CSV` | ✅ | ⛔ | ✅ | ⛔ |
...
```
증분이 되나        상세 호출에 허용량이 걸리는 소스만.  PET_TOUR 하나임
원본을 담나        담으면 FULL·INCREMENTAL 로 받고 LINK 로 넘김
                안 담으면 DIRECT 로 바로 보냄
```

`LINK` 와 `DIRECT` 가 정확히 반대라 **표시 하나로 양쪽이 다 설명됩니다.**
따로 두면 한쪽만 고쳐 어긋날 자리가 생기므로 `SourceType` 의 값 하나를 양쪽에서 봅니다.
...
| 증분을 지원하지 않는 소스에 `INCREMENTAL` | `501` `RUN_TYPE_NOT_SUPPORTED` |
| 원본을 담지 않는 소스에 `LINK` | `501` `RUN_TYPE_NOT_SUPPORTED` |
| 원본을 담는 소스에 `DIRECT` | `501` `RUN_TYPE_NOT_SUPPORTED` |
| 원본을 담지 않는 소스에 받아 오기 | `501` `RUN_TYPE_NOT_SUPPORTED` |
| 그 조합을 돌릴 구현이 없음 | `501` `COLLECTOR_NOT_REGISTERED` |

> ⛔**마지막 줄이 맨 아래인 것이 중요합니다.**
> 조합 검사를 전부 지난 뒤에 구현이 있는지 봅니다.
> 순서를 뒤집으면 원본을 담지 않는 소스에 받아 오기를 불렀을 때
> 「지원하지 않는 소스」로 거절됩니다. 그 소스는 지원하고 있고
> 맞지 않는 것은 실행 종류라 이유가 틀립니다.
> 두 응답은 상태 코드가 같아도 `code` 가 달라 부르는 쪽이 그것으로 가릅니다.

> ⚠**무엇을 보는지는 실행 종류마다 다릅니다.**
> `DIRECT` 는 변환기가 있는지 보고 나머지는 수집기가 있는지 봅니다.
> `MOIS_VET` 에는 수집기가 없어 수집기만 보면 `DIRECT` 가 아예 지나가지 못합니다.

> ⛔**`LINK` 와 `DIRECT` 는 `place` 가 떠 있어야 합니다.** 안 떠 있으면 첫 묶음에서 `502` `PLACE_LINK_FAILED` 로 끝납니다.
...
**`changedCount` 와 `progress` 가 이 기록의 핵심입니다.**
몇 건이 바뀌었는지와 오퍼레이션마다 몇 번 불렀는지가 남아, 사람이 이 둘을 보고 다음 실행을 부를지 정합니다.
관리자 화면은 같은 모양을 [5-7](#5-7-get-apiv1adminingestruns--관리자가-수집-기록-보기) 로 받습니다.
그쪽은 공사 API 를 부른 받아 오기만 보여 줍니다.

> ⚠**`errorMessage` 는 이름과 달리 오류가 아닌 것도 담습니다.**
> 건너뛴 항목 목록, 허용량으로 멈춘 오퍼레이션 이름, 표본 검증 결과가 여기로 옵니다.

> ⛔**`DIRECT` 실행은 `fetchedCount` 와 `changedCount` 가 0 으로 남습니다.** 실패가 아닙니다.
> 그 값들은 우리 표에 담은 건수인데 그 실행은 담지 않습니다.
> 몇 건을 보냈는지는 로그에 남고, 실제로 들어갔는지는 `place` 에서 봅니다.
...
| `total` | 그 상태이면서 장소에 이어진 문서가 모두 몇 건인지. 몇 번 더 부를지 판단하는 값 |
| `placeId` | 이 문서가 이어진 장소. `extract` 가 뽑은 조건을 장소 단위로 보낼 때 씀 |
| `payload` | ⛔문자열이 아니라 객체. 받는 쪽이 두 번 파싱하지 않게 |
| `contentHash` | 처리 결과를 되돌려 쓸 때 그대로 보냄 — [5-4](#5-4-patch-internalrawstatus--처리-결과-되돌려-쓰기) |

`extract` 는 `PENDING` 만 씁니다. 나머지 둘은 사람이 들여다볼 때 쓰는 값이고,
`FAILED` 는 부분 인덱스를 안 타서 조금 느립니다.

**⛔장소에 이어진 문서만 줍니다.** `extract` 는 뽑은 조건을 장소 단위로 `policy` 에 보내므로
장소가 없는 문서는 보낼 곳이 없습니다. `total` 도 같은 기준으로 셉니다.

| 목록에서 빠지는 문서 | 까닭 | 언제 나오나 |
|---|---|---|
| 고캠핑 8건 | 좌표도 주소도 없어 `place` 가 장소를 못 만듦 — [11-8](#11-8-넘겼는데-place_id-가-안-채워질-때) | 나오지 않음 |
| 수집 직후 문서 | 장소로 넘기기(`LINK`) 전이라 `place_id` 가 아직 비어 있음 | `LINK` 를 돌리면 그때부터 |
...
> 표시용 본문을 보지 않습니다. 17,463건을 100건씩 나눠 주는데 안 쓰는 필드가 실리면 응답만 커집니다.
...
```json
요청   { "done":   [ { "id": "01a082a9-1f74-…", "contentHash": "74ed7dd2…" },
                   { "id": "01a082a8-fba2-…", "contentHash": "3c9a01e4…" } ],
         "failed": [ { "id": "01a082a8-ce15-…", "contentHash": "b20f7c6d…" } ] }
응답   200   { "updated": 2, "skipped": 1 }
```

**묶어서 받습니다.** `extract` 가 100건을 가져가 처리하므로 되돌려 쓰는 것도 100건입니다.

**문서마다 처리할 때 본 내용 해시를 함께 받습니다.** 목록 조회로 받은 `contentHash` 를 그대로 보냅니다.

| 이런 때 | 응답 |
|---|---|
| 해시가 지금 것과 같음 | `200` · `updated` 로 셈 — 상태가 `DONE` · `FAILED` 로 바뀜 |
| 해시가 지금 것과 다름 | `200` · `skipped` 로 셈 — ⛔**상태를 그대로 둠**(`PENDING`) |
| 없는 식별자가 하나라도 섞임 | `400` `RAW_DOCUMENT_NOT_FOUND` ⛔**전체를 거절** |
| 같은 식별자가 `done` 과 `failed` 에 함께 | `400` `RAW_DOCUMENT_STATUS_CONFLICT` |
| 목록 원소가 `null` 이거나 `id` · `contentHash` 가 비어 있음 | `400` `VALIDATION_FAILED` |

**해시가 다른 것은 `extract` 가 가져간 사이에 재수집이 내용을 바꾼 것입니다.**
옛 내용으로 뽑은 결과를 `DONE` 으로 덮으면 새 내용이 다시 뽑히지 않습니다.

```
extract 가 가져감 (해시 A)
        │
        ├──  그사이 재수집이 내용을 바꿈   해시 B · 상태 PENDING
        │
extract 가 해시 A 로 done 을 보냄
        │
        ├──  해시를 안 보던 때     DONE 으로 덮임   ⛔새 내용 B 가 영영 안 뽑힘
        └──  지금                  PENDING 그대로   ✅다음 실행이 B 를 가져감 · skipped 로 셈
```

오류가 아니라서 거절하지 않고 건수로 알립니다. 건너뛴 문서의 식별자는 이 서비스 로그에 앞 10개만 남습니다.

**상태 칸 하나만 씁니다.** 문서를 불러 통째로 저장하면 모든 칸이 다시 쓰여,
그사이 재수집이나 장소 넘기기가 바꾼 원본 · `place_id` 를 옛 값으로 되덮을 수 있습니다.
해시 조건을 건 UPDATE 한 줄로 상태와 감사 칸만 바꾸므로 그 틈이 없습니다.
...
> ⛔**동물병원은 이 API 가 영영 빈 목록을 돌려줍니다.** 오류가 아니라 정상입니다.
> `MOIS_VET` 이 원본을 거치지 않기 때문이며 이유는 [2-6](#2-6-mois_vet--담지-않고-바로-보냅니다) 에 있습니다.
> **화면이 「이 장소는 원문 문서가 없습니다」를 보여주어야 합니다.**
> 그대로 두면 사용자에게는 고장으로 보입니다.

원본을 담는 세 소스가 같은 장소를 가리키면 셋이 다 나옵니다. 실제로 이렇게 나옵니다.
...
### 5-6. `POST /api/v1/admin/ingest/runs` — 관리자가 수집을 걸기

관리자 화면 「운영」의 「공사 데이터 최신 수집」 카드가 부릅니다.
한국관광공사 OpenAPI 를 그 자리에서 불러 바뀐 원문을 가져옵니다.

```json
요청   { "source": "PET_TOUR", "runType": "INCREMENTAL" }
응답   202   { "runId": "01a0c6aa-aac4-7560-a81d-310417282f0f" }
```

**받는 조합은 둘뿐입니다.** 둘 다 공사 API 를 가장 적게 부르는 받아 오기입니다.

| `source` | `runType` | 한 번에 부르는 수 |
|---|---|---|
| `PET_TOUR` | `INCREMENTAL` | 목록 11회 + 상세 세 오퍼레이션이 각각 (바뀐 곳 + 표본 20)회 |
| `GOCAMPING` | `FULL` | 목록 1회 — 한 번에 전량이 옴 |

그 밖의 조합은 400 `INGEST_RUN_NOT_ALLOWED` 로 거절합니다.

- 반려동물 동반여행 전량은 상세를 3천 회 넘게 불러 그날 허용량을 통째로 씁니다.
- `LINK` · `DIRECT` 는 장소 서비스로 넘기는 실행이라 이 입구의 범위를 넘습니다.
- 문화정보원 · 행정안전부는 공사 API 가 아니라 레포에 담은 파일을 읽습니다.

---

**수집은 원문 적재에서 끝납니다.**
장소로 넘기기(`LINK`)와 조건 추출(`extract`)로 이어지지 않습니다.
바뀐 원문은 처리 상태가 대기로 돌아가 다음 추출 때 다시 읽히고,
새로 생긴 원문은 사람이 넘기기를 부를 때 장소에 이어집니다.
...
| 순서 | 막는 것 | 응답 |
|---|---|---|
| 1 | 두 조합이 아닌 요청 | 400 `INGEST_RUN_NOT_ALLOWED` |
| 2 | 같은 소스의 받아 오기가 끝난 지 10분이 안 됨 | 429 `INGEST_COOLDOWN` |
| 3 | 같은 소스가 아직 도는 중 | 409 `INGEST_ALREADY_RUNNING` |

조합을 먼저 봅니다. 안 되는 조합에 「잠시 후 다시」가 나가면 기다려도 안 된다는 것을 알 수 없기 때문입니다.

10분 잠금은 다음처럼 셉니다.

- **소스마다 따로 셉니다.** 반려동물 동반여행을 막 받았어도 고캠핑은 걸 수 있습니다.
  실행 중 잠금도 소스마다이고, 허용량도 API 마다 따로입니다.
- **누가 걸었든 셉니다.** 관리자 · 매일 예약 · `/internal` 어느 쪽이 불러도 허용량은 같이 줄어듭니다.
  넘기기 · 바로 보내기는 바깥을 부르지 않아 세지 않습니다.
- **끝난 시각부터 셉니다.** 도는 중인 실행은 끝난 시각이 없어 잠금에 잡히지 않고 3 에서 막힙니다.
- 잠금 시간은 설정 `app.ingest.cooldown` 이 정합니다. 기본은 10분입니다 — [8-2](#8-2-appingest).

> ⚠**`/internal` 트리거에는 이 규칙이 없습니다.** 사람이 전량을 다시 받거나 넘기기를 부르는 자리라서입니다.
> 대신 10분 잠금을 셀 때는 그 실행도 함께 셉니다.

---

**권한이 없으면 공통 응답이 나갑니다.** 일반 계정은 403 `ACCESS_DENIED`, 로그인하지 않으면 게이트웨이가 401 을 돌려줍니다.

**실제로 불러 본 결과입니다** (2026년 9월 22일 · 로컬).

| 실행 | 걸린 시간 | 받음 | 바뀜 | 부른 수 |
|---|---|---|---|---|
| 반려동물 동반여행 증분 | 12초 | 3 | 3 | 목록 11 + 상세 23 × 3 = 80 |
| 고캠핑 목록 | 8초 | 3,004 | 52 | 1 |

곧바로 반려동물 동반여행을 다시 부르면 429, 반려동물 동반여행 전량은 잠금 중이어도 400,
고캠핑을 연달아 두 번 부르면 202 다음 409 가 나왔습니다.

---

### 5-7. `GET /api/v1/admin/ingest/runs` — 관리자가 수집 기록 보기

```
GET /api/v1/admin/ingest/runs?size=10
```
...
`source` 는 받지 않습니다. 보이는 것이 정해져 있기 때문입니다.

**공사 API 를 부른 받아 오기만 새것부터 보입니다.**
반려동물 동반여행 · 고캠핑의 `FULL` · `INCREMENTAL` 이고, 누가 걸었든(관리자 · 매일 예약 · `/internal`) 함께 보입니다.
넘기기 · 바로 보내기와 문화정보원 실행은 빠집니다.

- 카드 이름이 「공사 데이터 최신 수집」이라, 그 밖의 줄이 섞이면 받은 건수 0 인 줄이 끼어 뜻이 어긋납니다.
- 10분 잠금이 세는 실행과 같아서, 막혔을 때 그 까닭이 표 맨 위에 보입니다.
- 개발하며 돌린 반려동물 동반여행 전량도 실제 호출이라 이력으로 남습니다.

```json
{ "code": "SUCCESS", "message": "요청이 성공적으로 처리되었습니다.",
  "data": { "runs": [
    { "id": "01a0c6ac-25fa-7608-a976-d000e40d95e5",
      "source": "GOCAMPING", "runType": "FULL", "status": "DONE",
      "startedAt": "2026-09-22T10:12:45.818377", "finishedAt": "2026-09-22T10:12:53.395493",
      "fetchedCount": 3004, "changedCount": 52,
      "progress": { "basedList": { "count": 1, "cursor": null } },
      "errorMessage": null },
    { "id": "01a0c6aa-aac4-7560-a81d-310417282f0f",
      "source": "PET_TOUR", "runType": "INCREMENTAL", "status": "DONE",
      "startedAt": "2026-09-22T10:11:08.733444", "finishedAt": "2026-09-22T10:11:20.404339",
      "fetchedCount": 3, "changedCount": 3,
      "progress": { "detailIntro2": { "count": 23, "cursor": "947611" },
                    "detailCommon2": { "count": 23, "cursor": "947611" },
                    "detailPetTour2": { "count": 23, "cursor": "947611" },
                    "petTourSyncList2": { "count": 11, "cursor": null } },
      "errorMessage": null } ] },
  "traceId": "6ab1d66eca5a90fdcb73b421618fc5f1" }
```

위 반려동물 동반여행 증분은 **바뀐 곳 3 + 표본 20 = 상세 오퍼레이션마다 23회**를 불렀습니다.
원소의 칸은 [5-2](#5-2-get-internalingestruns--실행-이력) 와 같습니다.

> ⚠**`status` 는 다섯 가지입니다.** `RUNNING` · `DONE` · `FAILED` · `QUOTA_STOPPED` · `INTERRUPTED` 이고,
> 앞의 둘 말고는 [3-4](#3-4-실패를-네-가지로-가릅니다) 에 뜻이 있습니다.
> 서비스가 뜰 때 끝내지 못한 실행은 `FAILED` 로 마감되고 `errorMessage` 에 「기동 때 정리」가 남습니다 —
> [9-4](#9-4-그-소스가-이미-돌고-있다고-나올-때).
...
| `INGEST_COOLDOWN` | 429 | 관리자 입구 · 매일 예약에서, 같은 소스의 받아 오기가 끝난 지 잠금 시간이 안 됨 |
| `COLLECTOR_NOT_REGISTERED` | 501 | 그 소스의 수집기가 없음 |
| `RUN_TYPE_NOT_SUPPORTED` | 501 | 그 소스가 증분을 지원하지 않거나 원본을 담지 않음 |
| `INGEST_RUN_NOT_ALLOWED` | 400 | 관리자 입구에 두 조합이 아닌 요청이 옴 |
...
| `SOURCE_FILE_MALFORMED` | 500 | CSV 컬럼 수가 다르거나 꼭 있어야 하는 컬럼이 없음 |
...
에러는 `data` 가 비고 `code` 와 `message` 가 채워집니다. 실제 응답입니다.

```json
{ "code": "INGEST_RUN_NOT_ALLOWED",
  "message": "관리자 화면에서는 반려동물 동반여행 증분과 고캠핑 목록만 실행할 수 있습니다.",
  "data": null, "traceId": "6ab1d66e857d0355eb5cdf5f0d2b564f" }
```
...
```
raw_document     받아 온 것 하나가 한 줄        17,471줄
...
> **`MOIS_VET` 이 없는 것이 맞습니다.** 이 표는 `raw_document` 의 것인데 그 소스는 담기지 않습니다.
> 그쪽 식별자는 `관리번호` 이고 [2-6](#2-6-mois_vet--담지-않고-바로-보냅니다) 에 적어 두었습니다.
...
|---|---|---|---|
| 시설명만 | 12,278 | ⛔506개 | 29 |
| **시설명 + 지번주소** | **13,399** | 0개 | 52 |
| 시설명 + 도로명주소 | 13,406 | 30개 | 50 |
| 시설명 + 위도 + 경도 | 13,408 | 28개 | 52 |

> ⚠**시설명의 공백을 걷어낸 뒤의 숫자입니다.** 처음에는 그냥 이어 붙였는데
> 같은 가게가 「박영재 동물병원」과 「박영재동물병원」으로 들어와 키가 갈렸습니다.
> 아홉 쌍이 그랬고 전부 띄어쓰기 차이뿐이었습니다.
> 지번주소에는 걸지 않았습니다. 주소 띄어쓰기로 갈린 쌍이 한 건도 없었고,
...
| `run_type` | `FULL` · `INCREMENTAL` · `LINK` · `DIRECT` |
...
그 표가 생기기만 하고 언제나 비어 있습니다.** 이유는 [10-20](#10-20-kafka-를-안-쓰는-이유) 에 있습니다.
...
infrastructure   persistence(구현) · provider/external · provider/file
                 provider/internal · provider/convert · config · scheduler
```
...
관리자 입구와 매일 예약은 앞에 문지기를 하나 더 거칩니다.

```
AdminIngestController ──┐
                        ├──▶  IngestRunLauncher.launch
IngestScheduler ────────┘          │
                                   ├──  ① 두 조합이 아니면 400
                                   ├──  ② 같은 소스의 받아 오기가 끝난 지 10분이 안 됐으면 429
                                   │
                                   ├──▶  IngestTriggerService   @Transactional     ③ 실행 중이면 409
                                   │
                                   └──▶  IngestExecutor         @Async             위 사슬 그대로
```

**`IngestRunLauncher` 가 두 조합인지와 10분 잠금을 먼저 봅니다.**
통과하면 `/internal` 트리거와 같은 두 메서드를 차례로 부르고, 실행 중 검사는 원래대로 `startRun` 안에 있습니다.

**예약은 `launchScheduled` 로 들어옵니다.** 같은 검사를 거치되 ② 와 ③ 에 걸리면 예외를 던지지 않고 그 소스만 건너뜁니다.
두 소스 가운데 하나가 막혀도 다른 하나는 돌아야 하기 때문입니다.
...
`runType` 이 `DIRECT` 면 또 다른 실행기로 갑니다.
...
InternalIngestController
     ...
     ├──▶  IngestTriggerService   @Transactional     같은 것을 씀
     │
     └──▶  MoisVetDirectExecutor  @Async             ⛔트랜잭션 없음
                │
                ├──▶  CsvReader.read(...)                      행마다 콜백
                │          └──▶  영업 중인 것만 골라 냄
                │
                ├──▶  PlaceItemConverter.convert(...)           MOIS_VET 구현
                │          └──▶  Epsg5174Converter 로 좌표계를 옮김
                │
                ├──▶  PlaceLinkClient.send(...)                 1,000건씩 보냄
                │
                └──▶  ChunkWriter.complete / fail               마감은 같은 것을 씀
```

**돌려받은 식별자를 안 씁니다.** 채울 자리가 없기 때문입니다.
우리 표에 이 소스의 행이 없어 `PlaceLinkWriter` 를 부르지 않습니다.

**실행기가 셋인 이유는 시작점이 다르기 때문입니다.**

| 실행기 | 무엇에서 시작하나 | 어디로 |
|---|---|---|
| `IngestExecutor` | 바깥 API 나 파일 | `raw_document` |
| `PlaceLinkExecutor` | 우리 표 | `place` |
| `MoisVetDirectExecutor` | 파일 | `place` |

앞의 둘을 고쳐 담지 않았습니다.
수집 실행기는 넘겨받은 것을 원본 초안으로 만드는데 그 팩터리가 `MOIS_VET` 을 막고 있고,
넘기기 실행기는 우리 표를 읽는 것이 시작이라 읽을 행이 없습니다.

**마감을 수집 쪽 기록기에 맡깁니다.** 실행 기록을 다루는 방식이 같아 같은 코드를 두 번 쓸 이유가 없습니다.
넘기기 기록기는 묶음을 반영하는 일 하나만 맡습니다.
...
컨트롤러와 `IngestRunLauncher` 가 `startRun` 과 `execute` 를 차례로 부르는 것도 같은 이유입니다.
...
| `MoisVetDirectExecutor` | 파일을 읽어 바로 보내는 루프 |
| `IngestRunLauncher` | 관리자 입구와 매일 예약의 문지기. 두 조합 · 10분 잠금을 보고 실행을 걺 |
| `IngestScheduler` | 매일 04:00 두 소스를 차례로 걺. 켜기 스위치가 꺼져 있으면 아무것도 안 함 |
| `OrphanRunCleaner` | 뜨기 전에 시작돼 끝내지 못한 실행을 기동 때 실패로 마감 |
| `IngestQueryService` | 조회 넷. 관리자 수집 기록은 받아 오기만 추림 |
| `RawDocumentStatusService` | 상태 묶음 갱신 |
| `PetTourSampleVerifier` | 증분 전제를 표본으로 확인 |
| `JsonNormalizer` | 정규화 직렬화 + 해시 |
| `CsvReader` | 파일을 읽어 행마다 넘김. 소스 둘이 함께 씀 |
| `Epsg5174Converter` | 평면 좌표를 위경도로 옮김 |
| `SourceCollector` 구현 3개 | 소스마다 받아 오는 방법 |
| `PlaceItemConverter` 구현 4개 | 소스마다 넘기는 형태로 바꾸는 방법 |
...
file/       CsvReader                         ⛔소스 둘이 함께 씀
            CultureCsvCollector · CultureDisplayBodyAssembler
convert/    PetTourItemConverter · GoCampingItemConverter · CultureItemConverter
            MoisVetItemConverter · Epsg5174Converter
...
> ⚠**`CsvReader` 만 소스마다 나누지 않았습니다.** 읽는 방법이 똑같기 때문입니다.
> 문화정보원과 행정안전부가 인코딩과 컬럼 수만 다르고,
> 헤더를 읽고 행마다 넘기고 컬럼 수로 형식을 검사하는 일은 같습니다. 그 셋은 값이지 다른 로직이 아닙니다.
> 수집기와 클라이언트와 변환기를 나눈 것과는 다릅니다.
> 그쪽은 오류 코드 체계와 거르는 판정과 키 매핑이 실제로 달라
> 조건문으로 담으면 소스를 더할 때마다 그 메서드가 길어집니다.

> ⛔**`CsvReader` 는 컬럼 수만 보지 않고 꼭 있어야 하는 이름도 봅니다.**
...
| 변환기 4종 | 어느 키를 어느 칸에. ⛔**좌표 순서와 분류 자리** |
| 좌표계 변환 | ⛔**옮긴 값이 대한민국 범위 안인지.** 손으로 적은 파라미터를 검사 |
| 파일 읽기 | 인코딩 · 컬럼 수 · 꼭 있어야 하는 이름 |
| 바로 보내기 실행기 | 영업 상태 필터 · 건너뛰기 · 묶음 경계 · 마감과 실패 |
| 조회 · 상태 갱신 | 언제나 첫 쪽인지 · 부분 반영이 안 생기는지 · 원본이 안 새는지 |
| 검증기 | 표본이 실행 결과를 안 바꾸는지 |
| 관리자 입구 · 예약 | 두 조합 · 10분 잠금 · 검사 순서 · 예약이 막히면 건너뛰는지 · 한 소스가 실패해도 다음으로 가는지 |
| 기동 정리 | 뜨기 전에 시작된 실행만 실패로 마감하는지 · 기준 시각을 빈을 만들 때 정하는지 · 없으면 조용히 끝나는지 |
| 정렬 · 실행 기록 쿼리 | ⛔**PostgreSQL 컨테이너를 띄워** 확인. 받아 오기만 추리는지 · 끝난 시각으로 세는지 · 시작 시각으로 거르는지까지 |
...
**세 곳에서 실물을 띄웁니다.** 흉내로는 못 잡는 것이 있기 때문입니다.
...
실행 기록   10분 잠금과 관리자 기록이 쿼리 조건에 걸려 있어
            흉내로는 받아 오기만 추리는지 · 끝난 시각으로 세는지 알 수 없음
```

> ⛔**좌표계 시험이 특히 중요합니다.** 좌표계 정의를 문자열로 손수 적었는데,
> 그 값이 틀려도 예외가 나지 않고 결과가 몇십 미터 어긋나는 것으로 끝납니다.
> 실제 좌표를 넣어 옮긴 값이 대한민국 범위 안인지 보는 것 말고는 확인할 방법이 없습니다.
...
app:
  ingest:
    ...
    cooldown: 10m
    schedule:
      enabled: ${INGEST_SCHEDULE_ENABLED:false}
      cron: "0 0 4 * * *"
    ...
    mois-vet:
      file-path: data/mois/mois-vet-20260908.csv
      charset: CP949
```
...
| `cooldown` | 같은 소스의 받아 오기가 끝난 뒤 다시 걸 수 없는 시간 | 10분이면 한 소스가 하루 최대 144번. 반려동물 동반여행은 50번쯤 걸어야 그날 상세 1,000회에 닿음 |
| `schedule.enabled` | 매일 예약을 켤지 | ⛔기본 꺼짐. 배포 서버에서만 켬 — [9-2](#9-2-수집을-언제-부르나) |
| `schedule.cron` | 언제 돌지 | 매일 04:00. 시간대는 코드가 `Asia/Seoul` 로 고정 |
...
| `mois-vet.file-path` | 같음 | 같음 |
| `mois-vet.charset` | 파일 인코딩 | ⛔이 소스만 CP949. 문화정보원은 UTF-8 |

**`cooldown` 은 관리자 입구와 매일 예약에만 걸립니다.** `/internal` 트리거는 막지 않고 셀 때만 함께 셉니다 —
[5-6](#5-6-post-apiv1adminingestruns--관리자가-수집을-걸기).
50번은 바뀐 곳이 없을 때의 수입니다. 한 번에 표본 20곳 × 상세 오퍼레이션 셋이라 오퍼레이션마다 20회씩 쓰고,
바뀐 곳이 있으면 그만큼 더 씁니다.

> ⛔**인코딩을 코드에 박지 않고 값으로 둔 이유**는 그것이 소스가 정하는 것이기 때문입니다.
> 다음 판이 바뀌면 코드를 고치게 되고, 어긋나면 한글이 깨져 사업장명을 못 찾아
> 전량이 건너뛰어집니다. 그런데 오류가 안 나서 0건으로 조용히 끝납니다.

**⛔인증키 환경변수 이름에 소스가 안 들어갑니다.** 포털이 계정마다 키를 하나만 주기 때문입니다.
기상청도 같은 계정이라 소스 이름을 붙이면 그때 또 같은 고민을 하게 됩니다.
...
> ⚠**`cooldown` 과 `schedule` 은 없어도 기동이 됩니다.** 코드에 기본값(10분 · 꺼짐)이 있어서입니다.
> 그래도 사본을 맞춰 두고, 특히 예약은 시험에서 꺼 둔다는 것을 적어 둡니다.
> 켜진 채 새벽 4시에 시험이 돌면 가짜 인증키로 공사 API 를 부르려 합니다.
...
관리자 화면 「운영」의 「공사 데이터 최신 수집」 카드는 이 가운데 공사 API 를 부른 받아 오기만 보여 줍니다 —
[5-7](#5-7-get-apiv1adminingestruns--관리자가-수집-기록-보기).

---

### 9-2. 수집을 언제 부르나

**공사 API 두 소스는 매일 예약과 관리자 버튼이 부릅니다.** 나머지는 사람이 `/internal` 트리거로 부릅니다.

| 소스 | `runType` | 누가 · 얼마나 자주 |
|---|---|---|
| `PET_TOUR` | `INCREMENTAL` | 매일 04:00 예약 · 관리자 버튼. 전량이 필요하면 사람이 `/internal` 로 `FULL` |
| `GOCAMPING` | `FULL` | 매일 04:00 예약 · 관리자 버튼. 1회라 부담이 없음 |
| `CULTURE_CSV` | `FULL` | 파일을 바꿨을 때만. 사람이 `/internal` 로 |
| 위 소스 셋 | `LINK` | 위가 끝난 뒤 사람이 `/internal` 로. **받은 것이 바뀌었을 때만 의미가 있음** |
| `MOIS_VET` | `DIRECT` | 파일을 바꿨을 때만. **`LINK` 를 따로 안 부릅니다** |

> ⛔**소스마다 되는 실행 종류가 정해져 있습니다.** 어긋나면 501 로 거절합니다.
> 조합 표는 [5-1](#5-1-post-internalingesttrigger--수집을-시작) 에 있습니다.
> 관리자 입구는 그보다 좁아 두 조합만 받습니다 — [5-6](#5-6-post-apiv1adminingestruns--관리자가-수집을-걸기).

> **`MOIS_VET` 만 한 번으로 끝납니다.** 다른 소스는 받아 오기와 넘기기를 따로 부르는데
> 이 소스는 읽는 것과 보내는 것이 한 실행이기 때문입니다.

---

**매일 예약은 코드에 있고 켜기 스위치 뒤에 있습니다.** 기본은 꺼짐이고 배포 서버에서만 켭니다.

| 자리 | `INGEST_SCHEDULE_ENABLED` | 어떻게 되나 |
|---|---|---|
| 배포 서버 | `true` | 매일 04:00 두 소스를 차례로 걺 |
| 개발 PC | 넣지 않음 (꺼짐) | 공사 API 를 스스로 부르지 않음 |

**`.env` 에서 켭니다.** infra compose 의 `ingest-service` 블록이 이 값을 컨테이너로 넘기고,
`.env` 에 없으면 `false` 로 채웁니다. 설정 저장소의 `app.ingest.schedule.enabled` 가 이 환경변수를 읽습니다 —
[8-2](#8-2-appingest).

**기본이 꺼짐인 이유는 허용량이 되돌릴 수 없는 자원이기 때문입니다.**
개발 중에 켜져 있는 줄 모르고 돌면 그날 몫이 사라집니다. 켜졌는지는 기동 로그 한 줄로 봅니다.

```
예약 수집 켜짐. cron=0 0 4 * * * zone=Asia/Seoul sources=[PET_TOUR, GOCAMPING]
예약 수집 꺼짐. app.ingest.schedule.enabled=false — 배포 서버에서만 켭니다
```

**예약도 관리자 입구와 같은 규칙을 거칩니다.** 새벽 4시 직전에 누가 버튼을 눌렀으면
그 소스는 10분 잠금에 걸려 그날 예약을 건너뜁니다. 방금 받았으니 다시 받을 까닭이 없습니다.
건너뛴 것은 오류가 아니라 로그 한 줄로 남습니다.

**04:00 은 search 의 매일 재색인과 같은 시각입니다.**
이 수집은 원문 적재에서 끝나 장소와 검색에 닿지 않으므로 부딪히지 않습니다.
...
**자동으로 잇지 않습니다.** 받아 오기가 끝나면 `LINK` 를 따로 불러야 합니다.
자동으로 이으면 받아 오기만 다시 하고 싶을 때 넘기기까지 딸려 오고,
무엇을 언제 내보낼지를 사람이 정하지 못하게 됩니다.
매일 예약과 관리자 버튼도 받아 오기만 합니다. 새로 생긴 원문은 사람이 넘기기를 부를 때 장소에 이어집니다.
...
앱을 중간에 끄면 `RUNNING` 이 남습니다. **유일 인덱스의 대가입니다.** 그 소스의 다음 실행이 409 로 막힙니다.

**서비스가 뜰 때 저절로 치웁니다.** 기동이 끝나면 이 프로세스가 뜨기 전에 시작된 `RUNNING` 을 `FAILED` 로 마감합니다.
뜬 뒤에 들어온 요청이나 예약이 건 실행은 건드리지 않습니다 — [10-24](#10-24-끝내지-못한-실행을-기동-때-실패로-마감하는-이유).
`error_message` 에 「기동 때 정리 — 앞 프로세스가 끝내지 못한 실행」 이 남아 손으로 마감한 것과 구별되고,
로그에도 한 줄이 찍힙니다. 남아 있던 것이 없으면 이렇게 나옵니다.

```
기동 때 정리 — 끝내지 못한 실행이 없습니다
```

있었으면 경고로 건수와 실행 id 가 찍힙니다. **그래서 대개는 서비스를 다시 띄우면 끝납니다.**
IntelliJ 로 띄웠으면 다시 실행하고, compose 로 띄웠으면 `infra` 폴더에서 다시 시작합니다.
...
> ⛔**한 대로 도는 것이 전제입니다.** 두 대가 뜨면 나중에 뜬 쪽이 다른 쪽이 돌리는 실행까지 실패로 마감합니다.
> 여러 대가 필요해지면 실행마다 「살아 있음」 신호를 남기는 방식으로 바꿔야 합니다.

**띄우지 않고 치워야 할 때만 손으로 합니다.** `infra` 폴더에서 그대로 부르면 됩니다.
...
**행정안전부 판도 절차가 같습니다.** 다만 두 가지가 다릅니다.

```
① 받는 길이 다름      data.go.kr 15045050 → [바로가기]
                    → 「인허가정보_동물」 → 「동물_동물병원」 → [전국 파일 다운로드]
                    ⛔직접 주소는 403 이고 [지역별] 은 열일곱 개를 합쳐야 함
② 인코딩이 CP949     설정의 app.ingest.mois-vet.charset 이 그것을 정함
                    바뀌면 그 값도 함께 고칠 것
③ 매일 갱신됨        문화정보원은 사실상 고정이지만 이쪽은 날마다 바뀜
                    그래서 「새 판이 나왔는지」가 아니라 「언제 받을지」가 문제임
```

> ⛔**`관리번호` 가 판이 바뀌어도 유지되는지는 아직 확인하지 못했습니다.**
> 인허가 시점에 부여되는 값으로 보여 유지될 가능성이 높지만 대조해 본 적이 없습니다.
> 새 판을 받으면 같은 병원 몇 곳의 `관리번호` 를 옛 판과 견주어 보십시오.
>
> 만약 바뀌어도 장소가 통째로 새로 만들어지지는 않습니다.
> 받는 쪽이 주소와 이름으로 다시 붙이기 때문이며, 대신 안 쓰이는 연결이 남습니다.
...
**로컬 평소 조합에는 넣지 않아도 됩니다.** 예약이 꺼져 있어 띄워 두어도 스스로 하는 일이 없습니다.
관리자 화면의 수집 카드나 장소 상세의 원문보기를 볼 때 함께 띄웁니다.

**배포에서는 늘 떠 있어야 합니다.** 관리자 버튼 · 매일 예약 · 원문보기가 이 서비스를 부릅니다.
...
**식별자만 받지 않고 내용 해시를 함께 받습니다.** 식별자만으로는 `extract` 가 어느 내용을 처리했는지 알 수 없어,
가져간 사이에 재수집이 바꾼 새 내용을 옛 결과로 덮게 됩니다 — [5-4](#5-4-patch-internalrawstatus--처리-결과-되돌려-쓰기)
...
나머지 둘에 증분을 받아 주고 안에서 전량을 돌게 하면 부르는 쪽이 소스마다 실행 종류를 가리지 않아도 되지만,
**같은 일을 두 이름으로 부르게 되고 코드만 보아서는 구별이 안 됩니다.**
매일 예약은 소스가 증분을 지원하는지를 보고 실행 종류를 스스로 고릅니다(`IngestRunLauncher.routineRunType`).
...
### 10-17. 소스 하나만 원본을 안 담는 이유

**담아 둘 값이 없기 때문입니다.**

원본을 보관하는 까닭이 둘인데 행정안전부 동물병원은 둘 다 해당하지 않습니다.
스물다섯 개 컬럼이 전부 사업장명·주소·영업상태·좌표·전화·면적이라
`extract` 가 해석할 문장도, 원문보기에 보여줄 자연어도 없습니다.
...
표시용 본문을 만들면
     ...
     [영업상태] 영업/정상
     [인허가일자] 2015-03-12
⛔이것은 원문이 아니라 우리가 만든 표임
```

**원문보기의 목적과도 맞지 않습니다.**
그 화면이 있는 이유는 「우리 판정의 근거를 직접 보는 것」인데
이 소스에는 판정에 쓰이는 값이 하나도 없습니다.
동반 조건은 문화정보원이 주고 이쪽은 폐업 여부와 좌표만 줍니다.

> ⚠**한때 근거가 흔들린 적이 있습니다.** 처음에는 「`extract` 가 뽑을 것이 없다」가 근거였는데
> 원문보기라는 쓰임이 나중에 생기면서 그 근거의 절반이 사라졌습니다.
> 다시 보고 「보여줄 자연어 자체가 없다」로 바꿨고 결론은 같았습니다.

⬜**그래서 동물병원은 원문보기가 영영 빈 목록입니다.**
오류가 아니라 정상이며 화면이 그것을 안내해야 합니다.

---

### 10-18. 좌표계 변환을 이쪽이 하는 이유

**받는 쪽에 맡기면 측량 좌표가 버려집니다.**

```
place 가 좌표를 받으면        대한민국 범위 안인지부터 봄
                           위도 33.0~38.7 · 경도 124.5~132.0
⛔미터 단위 평면 좌표         "199947.178" 은 그 범위에 안 들어감
                           → "쓸 수 없는 좌표" 로 판정
                           → 주소 지오코딩으로 넘어감
결과                       소스가 준 측량 좌표 5,258건이 통째로 버려짐
                           지오코딩 좌표는 도로변에 찍혀 오차가 훨씬 큼
```

**데이터베이스 함수를 쓸 자리도 없습니다.**
`PostGIS` 로 옮기면 된다고 보았으나 그러려면 값이 이미 표에 들어가 있어야 합니다.
받는 쪽은 자바에서 판정한 뒤에 저장하는 구조라 그 사이에 함수가 낄 곳이 없습니다.

**좌표계 변환은 파싱의 일부로 봅니다.** 소스가 준 값을 읽을 수 있는 형태로 만드는 일입니다.
이름을 다듬거나 주소를 표준화하는 것과는 다릅니다. 그쪽은 받는 쪽의 규칙입니다.

---

### 10-19. 좌표계 정의를 문자열로 적은 이유

`proj4j` 코어에는 EPSG 번호로 찾는 기능이 없고 그 데이터셋이 별도 아티팩트입니다.

```
proj4j-epsg 를 넣으면    "EPSG:5174" 문자열로 끝남
                       ⛔대신 EPSG 데이터셋 전체가 이미지에 실림
                         우리가 쓰는 좌표계는 하나뿐임

직접 구현하면           ⛔Bessel 타원체 횡메르카토르 역투영을 손으로 짜는 일
                         틀려도 예외가 안 나고 값이 몇십 미터 어긋나는 것으로 끝남
```

**손으로 적은 값이 맞는지는 확인할 방법이 있습니다.**
착수 전에 같은 파일을 파이썬으로 옮겨 기존 장소와 거리를 재 두었고,
그 결과와 견주면 파라미터가 어긋났는지 바로 드러납니다.
틀리면 거리가 통째로 벌어지므로 조용히 지나가지 않으며 시험이 그것을 검사합니다.

---

### 10-20. Kafka 를 안 쓰는 이유
...
그래서 템플릿에서 따라온 Kafka 스타터와 테스트 짝을 뺐고,
앱 클래스가 공통 모듈의 Outbox 엔티티 · 레포지터리를 찾던 스캔 두 줄도 지웠습니다.
공통 모듈의 발행 자동 설정은 JPA 와 Kafka 가 함께 있을 때만 켜지므로 이제 그 중계가 뜨지 않습니다.

`outbox` · `processed_event` 표는 공통 마이그레이션이 만들어 두어 그대로 있고 언제나 비어 있습니다.
이벤트가 필요해지면 `build.gradle` 의 두 줄과 앱 클래스의 스캔 두 줄을 함께 되살립니다.

Redis 스타터와 회로 차단기 스타터도 같은 이유로 뺐습니다. 쓰지 않는 Redis 가 의존성에 있으면
상태 확인이 그 연결을 시도해 Redis 가 없는 환경에서 unhealthy 가 되고,
재시도는 [10-16](#10-16-재시도를-애노테이션으로-안-한-이유) 처럼 손으로 짰습니다.


---

### 10-21. 예약을 코드에 두고 기본으로 꺼 둔 이유

**처음에는 스케줄러를 코드에 두지 않기로 했습니다.** 개발 중에 켜져 있는 줄 모르고 돌면
되돌릴 수 없는 허용량이 그날 몫만큼 사라지기 때문이었고, 그래서 Jenkins 잡이 부르기로 했습니다.

**그 까닭은 스위치로 지키고 예약은 코드로 옮겼습니다.** 스위치는 기본이 꺼짐이고 배포 서버에서만 켭니다.
켜졌는지는 기동 로그 한 줄로 보입니다 — [9-2](#9-2-수집을-언제-부르나).

| 다른 자리 | 두지 않은 까닭 |
|---|---|
| Jenkins 잡 | Jenkins 서버가 아직 없고, 작은 배포 서버에 올리기에는 무겁습니다 |
| 배포 서버의 cron | 두 조합과 10분 잠금을 부르는 쪽에 따로 걸어야 하고, 설정이 레포 밖에 흩어집니다 |

**코드에 두면 규칙이 한 곳에 모입니다.** 매일 예약과 관리자 버튼이 같은 `IngestRunLauncher` 를 거치므로
두 조합 · 10분 잠금 · 원문 적재까지만이라는 규칙을 한 번만 적습니다.

**두 소스를 다 매일 겁니다.** 고캠핑은 목록 1회라 부담이 없고, 두 소스의 원문이 함께 새것으로 유지됩니다.

> ⚠**스케줄 켜기(`@EnableScheduling`)를 이 서비스가 직접 겁니다.** 공통 모듈의 것은 Kafka 가 있을 때만 켜지는데
> 이 서비스는 Kafka 를 뺐습니다 — [10-20](#10-20-kafka-를-안-쓰는-이유).
> 이것이 없으면 `@Scheduled` 가 오류 없이 무시되어 예약이 조용히 안 돕니다.

---

### 10-22. 관리자 입구가 두 조합만 받는 이유

**관리자 입구는 누가 눌러도 안전해야 합니다.** 관리자 계정만 있으면 코드를 몰라도 누를 수 있기 때문입니다.

| 막는 조합 | 까닭 |
|---|---|
| 반려동물 동반여행 전량 | 상세를 3천 회 넘게 불러 그날 허용량을 통째로 씁니다 |
| 넘기기 (`LINK`) · 바로 보내기 (`DIRECT`) | 장소 서비스로 넘기는 실행이라 원문 적재에서 끝난다는 약속을 넘습니다 |
| 문화정보원 · 행정안전부 | 공사 API 가 아니라 레포에 담은 파일을 읽어 새로 받을 것이 없습니다 |
| 고캠핑 증분 | 이 소스는 증분이 없습니다 |

**400 으로 알립니다.** 값은 우리가 정의한 것이고 `/internal` 로는 되는 조합이라, 이 경로에서만 안 된다는 뜻을 담았습니다.

| 쓰지 않은 응답 | 까닭 |
|---|---|
| 501 `RUN_TYPE_NOT_SUPPORTED` | 「아직 구현하지 않음」이라는 뜻인데 반려동물 동반여행 전량은 구현돼 있습니다 |
| 403 | 이 프로젝트에서 403 은 역할이나 탈퇴 계정 자리라 로그에서 권한 문제로 읽힙니다 |

**조합을 가장 먼저 봅니다.** 안 되는 조합에 「잠시 후 다시」가 나가면 기다려도 안 된다는 것을 알 수 없습니다.

---

**기록도 공사 API 를 부른 받아 오기만 보여 줍니다.** 카드 이름이 「공사 데이터 최신 수집」이라,
넘기기나 문화정보원 줄이 섞이면 받은 건수 0 인 줄이 끼어 읽기 어렵습니다.
10분 잠금이 세는 실행과 같은 줄이라 막혔을 때 그 까닭이 표 맨 위에 보입니다.

| 쓰지 않은 범위 | 까닭 |
|---|---|
| 실행 전부 | 넘기기 · 바로 보내기 줄이 섞입니다 |
| 관리자가 건 두 조합만 | `/internal` 로 건 전량이 잠금에는 세지는데 표에는 안 보여 막힌 까닭을 찾을 수 없습니다 |

---

### 10-23. 10분 잠금을 소스마다 실행 기록으로 세는 이유

**버튼을 여러 사람이 번갈아 눌러도 그날 허용량이 바닥나지 않게 하려는 것입니다.**
10분이면 한 소스가 하루 최대 144번 돌고, 반려동물 동반여행은 50번쯤 걸어야 그날 상세 1,000회에 닿습니다.

**소스마다 셉니다.** 허용량이 API 마다 따로이고 실행 중 잠금도 소스마다입니다.
한 덩어리로 세면 반려동물 동반여행 직후에 고캠핑이 막히고, 새벽 4시 예약도 두 소스의 시각을 벌려야 합니다.

**누가 걸었든 셉니다.** 관리자 · 매일 예약 · `/internal` 어느 쪽이 불러도 허용량은 같이 줄어들기 때문입니다.
넘기기 · 바로 보내기는 공사 API 를 부르지 않아 세지 않습니다.

**실행 기록 표로 셉니다.** 끝난 시각(`finished_at`)이 이미 남아 있고, 이 서비스는 Redis 를 쓰지 않습니다.
도는 중인 실행은 끝난 시각이 없어 여기 잡히지 않고 실행 중 검사가 막습니다.

**429 로 알립니다.** 도는 것이 없는데 막는 것이라 409 「이미 돌고 있음」과 뜻이 다릅니다.
409 를 쓰면 화면이 「이미 수집이 돌고 있습니다」로 틀리게 안내합니다.
`auth` 의 메일 다시 보내기와 `user` 의 요약 다시 만들기가 같은 429 를 씁니다.

**예약도 같은 잠금을 거칩니다.** 걸리면 오류로 끝내지 않고 그 소스만 건너뜁니다. 방금 받았으니 다시 받을 까닭이 없습니다.

> ⚠**검사와 생성 사이에는 잠금을 걸지 않습니다. 알고 둔 틈입니다.**
> 10분 잠금 검사와 실행 생성 사이에 같은 소스의 실행이 시작부터 끝까지 다 돌면 10분 안에 새 실행이 하나 더 생깁니다.
> 두 검사는 한 요청 안에서 잇달아 부르는 호출이라 몇 ms 차이이고,
> 가장 짧은 실행도 공사 API 를 한 번은 불러 몇 초가 걸리므로 실제로는 닿기 어렵습니다.
> 새더라도 받아 오기가 한 번 더 돌 뿐이고, 같은 소스가 나란히 도는 것은 실행 중 유일 인덱스가 여전히 막습니다.
> 소스별 데이터베이스 잠금으로 닫을 수는 있지만 막는 것이 허용량 한두 번이라 두지 않았습니다.

---

### 10-24. 끝내지 못한 실행을 기동 때 실패로 마감하는 이유

**끝내지 못한 실행은 프로세스가 죽을 때만 생깁니다.** 그러니 다시 뜰 때가 치울 때입니다.
두면 그 소스는 버튼이 계속 409 이고 매일 예약도 조용히 건너뜁니다.

**`FAILED` 로 마감합니다.** 재개 지점을 물려받는 것은 스스로 멈춘 `QUOTA_STOPPED` · `INTERRUPTED` 뿐이라,
까닭을 모르는 멈춤은 처음부터 다시 받는 편이 안전합니다. 증분과 고캠핑은 다시 받아도 몇십 회입니다.

**이 프로세스가 건 실행은 건드리지 않습니다.** 스프링 부트는 웹 서버를 열고 예약 수집을 등록한 뒤에 기동 완료 이벤트를 보냅니다.
그 사이에 들어온 요청이나 예약이 만든 실행까지 마감하면 그 실행은 멈추지 않고 계속 돌고,
실행 중 자리가 비어 같은 소스의 수집이 하나 더 나란히 돌 수 있습니다.
그래서 정리를 맡은 빈을 만든 시각보다 먼저 시작한 `RUNNING` 만 마감합니다.
빈은 웹 서버와 예약보다 먼저 만들어지므로 이 프로세스가 건 실행은 언제나 그 뒤에 시작합니다.

**한 대로 도는 것이 전제입니다.** 이 프로세스가 뜨기 전에 시작한 `RUNNING` 은 전부 앞 프로세스의 것이기 때문입니다.
두 대가 뜨면 나중에 뜬 쪽이 다른 쪽이 돌리는 실행까지 마감하므로,
그때는 실행마다 「살아 있음」 신호를 남기는 방식으로 바꿔야 합니다.

| 쓰지 않은 방법 | 까닭 |
|---|---|
| 실행을 걸 때 N분 넘은 것을 정리 | N 을 정해야 하고, 길게 도는 전량과 죽은 실행을 시간만으로 가를 수 없습니다 |
| 지금처럼 SQL 로 손으로 | 관리자 화면에서는 풀 수 없습니다 |
...
| `app.ingest.mois-vet 설정이 필요합니다` | 같음. 설정을 더할 때마다 그 파일도 함께 고칠 것 |
...
앱을 중간에 껐을 때입니다. **대개는 서비스를 다시 띄우면 풀립니다.** 뜰 때 남아 있던 실행을 실패로 마감하기 때문입니다.
띄우지 않고 풀어야 할 때만 [9-4](#9-4-그-소스가-이미-돌고-있다고-나올-때) 의 SQL 을 씁니다.
...
그대로 두면 매번 전량이 `PENDING` 이 되고 `extract` 가 17,471건을 다시 돕니다.
**한 건마다 생성형 모델을 부르므로 돈과 시간이 그만큼 듭니다.**

---

### 11-6. 동물병원이 하나도 안 들어갈 때

```
증상   DIRECT 실행이 DONE 인데 place 에 아무것도 안 들어감
      로그에 "읽음=10618 영업중=0 보냄=0" 으로 남음
```

**컬럼 이름이 바뀐 것입니다.** 소스가 판을 바꾸면서 이름을 고치면 이렇게 됩니다.

```
영업상태명 이 바뀜    모든 행이 비영업으로 판정됨
관리번호 가 바뀜      식별자가 없어 전부 건너뜀
사업장명 이 바뀜      변환기가 전부 비움
```

지금은 `CsvReader` 가 이 셋을 검사해 `SOURCE_FILE_MALFORMED` 로 접습니다.
그래도 「영업중=0」이 보이면 파일의 헤더를 눈으로 확인하십시오.

```powershell
Get-Content data\mois\mois-vet-20260908.csv -Encoding ([System.Text.Encoding]::GetEncoding(949)) -TotalCount 1
```

```bash
head -1 data/mois/mois-vet-20260908.csv | iconv -f CP949 -t UTF-8
```

> ⛔**한글이 깨져 보이면 인코딩이 어긋난 것입니다.**
> 설정의 `app.ingest.mois-vet.charset` 이 `CP949` 인지 보십시오.
> 어긋나면 사업장명을 못 찾아 전량이 건너뛰어지는데 오류가 나지 않습니다.

---

### 11-7. `place` 에 넘기다 `Read timed out` 이 날 때
...
⛔place 로 넘기기는 여러 번 돌려도 됨
    ...
    DIRECT 도 같음.  관리번호가 그대로라 장소가 다시 만들어지지 않음

⛔DIRECT 는 실행 기록의 건수를 못 믿을 것
    fetchedCount 와 changedCount 가 0 으로 남음.  우리 표에 담지 않기 때문임
    ✅몇 건을 보냈는지는 로그에, 실제로 들어갔는지는 place 에서 봄

⛔부르는 쪽과 불리는 쪽을 같은 방식으로 띄울 것
    컨테이너로 띄운 서비스는 컨테이너 내부 주소로 등록됨
    ⛔호스트에서 도는 쪽이 그 주소에 닿지 못해 Connect timed out 이 남
    ✅둘 다 IntelliJ 이거나 둘 다 컨테이너로 맞출 것
    *Read timed out 과 Connect timed out 은 원인이 완전히 다르므로 먼저 가릴 것
...
### 11-12. 매일 예약이 안 돈 것 같을 때

**기동 로그부터 봅니다.** `infra` 폴더에서 부릅니다.

```powershell
docker compose logs ingest-service | Select-String "예약 수집"
```

```bash
docker compose logs ingest-service | grep "예약 수집"
```

| 보이는 것 | 까닭 | 할 일 |
|---|---|---|
| 「예약 수집 꺼짐」 | 켜기 스위치가 꺼져 있음 | 배포 서버 `.env` 에 `INGEST_SCHEDULE_ENABLED=true` 를 넣고 다시 띄움 |
| 「켜짐」인데 04:00 기록이 없음 | 10분 잠금이나 실행 중에 걸려 건너뜀 | 「예약 수집을 건너뜁니다」 줄에 까닭이 있음 |
| 「예약 수집을 걸지 못했습니다」 | 그 밖의 오류 | 같은 줄의 예외를 봄. 다음 소스는 그대로 걸림 |

> ⚠**스위치를 켰는데도 「꺼짐」이면 compose 가 값을 넘기는지 봅니다.**
> `ingest-service` 블록의 `environment` 에 `INGEST_SCHEDULE_ENABLED` 줄이 없으면 `.env` 에 넣어도 컨테이너에 닿지 않습니다.
...
**다른 서비스가 생겨야 열리던 것은 전부 열렸습니다.** `extract` 가 담아 둔 원문을 가져가 조건으로 바꿉니다.

**소스 넷이 전부 들어왔습니다.** 받아 오기도 넘기기도 바로 보내기도 다 돕니다.
아래 숫자는 매일 예약을 켜기 전에 센 것이라, 예약이 돌면 새로 생긴 장소만큼 늘어납니다.

```
raw_document          17,471건
     CULTURE_CSV 13,399 · GOCAMPING 2,993 · PET_TOUR 1,079

place 로 넘어간 것      17,463건에 place_id 가 채워짐
     ⛔고캠핑 8건은 좌표도 주소도 없어 장소를 못 만든 것임

바로 보낸 것           MOIS_VET 5,474건 중 5,451건이 붙음
     ⛔23건은 좌표도 주소도 없어 장소를 못 만든 것임
```

**받아 온 것을 손보는 일은 전부 끝났습니다.** 한동안 네 가지가 남아 있었습니다.

| 무엇이었나 | 어떻게 됐나 |
|---|---|
| `source_id` 를 더 길게 | ✅안 해도 됐음. 실측 최대가 52자인데 폭이 100자 |
| 문화정보원 키에 이름 정규화 | ✅했음. 시설명 공백을 걷어내 아홉 건이 하나로 합쳐짐 |
| 문화정보원 주차 열 확인 | ✅확인했더니 이미 들어가 있었음. 9,776건 |
| `SH04` 8,611곳 | ✅안 담기로 정함. [12-2](#12-2-판단이-남은-것) 참고 |

> ⚠**주차 열은 기록이 사실과 반대였습니다.** 「13,408건이 빠졌다」고 적혀 있었는데
> 실제로는 문화정보원 주차가 처음부터 담겨 있었습니다.
> 비어 있는 것은 고캠핑인데 그 소스에 정형 주차 필드가 아예 없습니다.
> 문장에서 낱말을 찾아 채우는 것은 추측이라 하지 않습니다.
...
| ⬜인증키를 여러 개 쓸지 | 기상청이 붙어 하루 총 호출량이 나온 뒤 |
| ⬜행정안전부 판을 언제 받을지 | 아래 |
| ⬜`SH04` 독립 매장 3,455곳 | 아래 |

**행정안전부 판이 매일 갱신되는데 받는 절차가 없습니다.**

```
지금        레포에 커밋한 판을 씀.  받은 날에 얼어붙음
⛔문제      "지금 영업 중인 목록" 이 이 소스를 쓰는 이유의 절반인데 그 절반이 묶임
           문화정보원(2022~2025)보다는 훨씬 새 것이라 당장은 견딜 만함

다시 볼 시점   실제 사용자가 생길 때
검토할 것     같은 데이터의 OpenAPI 판(15045050 이 아니라 15154952)
            그쪽으로 가면 수집기만 갈아끼우면 되고 소스 값은 그대로임
```

---

**`SH04` 를 안 담기로 정했습니다.** 한때 근거가 약했는데 전수로 확인했습니다.

```
목록에서 lclsSystm2 가 SH04 인 8,611곳을 「면세점」으로 알고 통째로 뺐음
⛔실물은 백화점 · 아울렛 · 몰 · 화장품 · 마트 · 안경 · 약국이었음
  코드표는 맞았는데 그 코드가 분류 안 된 것을 몰아넣는 자리로 쓰이고 있었음

✅전수 8,611곳을 실제로 받아 보니
  전부 contenttypeid 가 38(쇼핑)이고 동반 조건 칸이 통째로 비어 있었음
  ⛔그중 5,156곳(59.9%)이 몰이나 아울렛에 입점한 점포임
    「스타필드 3층 ○○」 같은 것이라 장소 단위와 맞지 않음

→ "조건이 없어서 안 담는다" 가 아니라
  "담을 화면 자리가 생길 때까지 보류한다" 가 정확한 표현임
```

⬜**나머지 3,455곳은 독립 매장이라 다시 볼 자리입니다.**
그쪽은 장소 단위와 맞지만 **지금 서비스 분류에 쇼핑 대역이 없습니다.**
화면에 그 칸을 둘지 정해진 뒤에 판단합니다.
...
| **수집 실행** | 트리거 · 관리자 입구 · 매일 예약이 한 번 건 단위. `ingest_run` 한 줄 |
| **받아 오기** | 소스에서 원문을 받아 우리 표에 담는 실행. `FULL` · `INCREMENTAL` |
...
| **넘기기** | 담아 둔 것을 `place` 로 보내는 것. `runType: LINK` |
| **바로 보내기** | 담지 않고 파일을 읽어 그 자리에서 `place` 로 보내는 것. `runType: DIRECT` |
| **관리자 입구** | 관리자 화면이 게이트웨이를 거쳐 부르는 경로 둘. `/api/v1/admin/ingest/runs` · ADMIN 만 |
| **매일 예약** | 매일 04:00 반려동물 동반여행 증분과 고캠핑 목록을 거는 것. 켜기 스위치가 꺼져 있으면 안 돎 |
| **켜기 스위치** | 매일 예약을 켤지 정하는 환경변수. `INGEST_SCHEDULE_ENABLED` · 기본 꺼짐 |
| **10분 잠금** | 같은 소스의 받아 오기가 끝난 뒤 다시 걸 수 없는 시간. `app.ingest.cooldown` · 429 |
| **기동 때 정리** | 서비스가 뜰 때, 그 전에 시작돼 끝내지 못한 `RUNNING` 을 `FAILED` 로 마감하는 것 |
...
| **평면 좌표** | 지구를 평면에 편 뒤 미터로 재는 좌표. 위경도가 아님. `EPSG:5174` |
...
| **유레카** | 서비스 이름으로 주소를 찾아 주는 장부. 게이트웨이가 관리자 입구를 이 이름으로 찾음 |
