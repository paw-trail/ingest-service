...
| `OrphanRunCleaner` | 기동 때 끝내지 못한 실행을 실패로 마감 |
...
| 기동 정리 | 도는 중이던 실행을 전부 실패로 마감하는지 · 없으면 조용히 끝나는지 |
| 정렬 · 실행 기록 쿼리 | ⛔**PostgreSQL 컨테이너를 띄워** 확인. 받아 오기만 추리는지 · 끝난 시각으로 세는지까지 |
...
**서비스가 뜰 때 저절로 치웁니다.** 기동이 끝나면 남아 있던 `RUNNING` 을 전부 `FAILED` 로 마감합니다.
...
**끝내지 못한 실행은 프로세스가 죽을 때만 생깁니다.** 그러니 다시 뜨는 순간이 치울 때입니다.
...
**한 대로 도는 것이 전제입니다.** 뜨는 순간의 `RUNNING` 은 전부 앞 프로세스의 것이기 때문입니다.
...
| **기동 때 정리** | 서비스가 뜰 때 끝내지 못한 `RUNNING` 을 `FAILED` 로 마감하는 것 |
